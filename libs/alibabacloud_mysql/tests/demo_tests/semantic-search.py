...
def test_embedding_generation(embeddings: Embeddings, documents: List[Document]) -> None:
...
def test_add_documents(vector_store: AlibabaCloudMySQL, documents: List[Document]) -> List[str]:
...
def test_similarity_search(vector_store: AlibabaCloudMySQL, query: str, k: int = 5) -> List[Document]:
...
def test_search_by_vector(vector_store: AlibabaCloudMySQL, embeddings: Embeddings, query: str) -> List[Document]:
...
def test_retriever_batch(vector_store: AlibabaCloudMySQL, queries: List[str]) -> None:
    ...
    print(f"Creating retriever with search_type='similarity', k=1...")
...
async def test_async_similarity_search(
    ...
    concurrent_results = await asyncio.gather(
        *[vector_store.asimilarity_search(q, k=k) for q in queries]
    )
    ...
    if concurrent_time < sequential_time:
        ...
        print(f"   ✓ Async is truly non-blocking!")
        print(f"   ✓ Concurrent execution is {speedup:.2f}x faster than sequential")
        print(f"   ✓ This confirms async calls don't block each other\n")
    else:
        print(f"   ⚠ Note: Concurrent time ({concurrent_time:.4f}s) >= Sequential time ({sequential_time:.4f}s)")
        print(f"   This may indicate the operations are CPU-bound or very fast\n")
...
def main() -> None:
    ...
    try:
        ...
        try:
            ids = test_add_documents(vector_store, all_splits)
        except Exception as e:
        ...
        try:
            ...
            async def run_async_test():
                try:
                    ...
                    try:
                        if hasattr(vector_store, "_async_pool") and vector_store._async_pool:
                            vector_store._async_pool.close()
                            await vector_store._async_pool.wait_closed()
                            vector_store._async_pool = None
                            await asyncio.sleep(0.1)  # Brief delay to allow connections to be released
                    except Exception as e:
