...
def load_and_split_document(file_path: str) -> list[Document]:
    ...
    print(f"  Regular paragraphs: {sum(1 for d in documents if not d.metadata.get('is_footnote'))}")
    
    return documents


def test_simple_similarity_search(vector_store: AlibabaCloudMySQL, query: str, k: int = 5) -> None:
...
def test_metadata_filter_equality(vector_store: AlibabaCloudMySQL, query: str, k: int = 5) -> None:
    ...
    print(f"Query: '{query}'")
    print(f"Filter: {{'is_footnote': True}}")
    print(f"Returning top {k} results...\n")
    ...
    for i, doc in enumerate(results, 1):
        ...
        assert doc.metadata.get("is_footnote") == True, "Filter failed!"
        print()
...
def test_metadata_filter_operators(vector_store: AlibabaCloudMySQL, query: str) -> None:
    ...
    results = vector_store.similarity_search(
        query,
        k=5,
        filter={"paragraph_id": {"$gte": 5, "$lte": 10}}
    )
    print(f"✓ Found {len(results)} results")
    for doc in results:
...
def test_combined_filters(vector_store: AlibabaCloudMySQL, query: str) -> None:
    ...
    print(f"Query: '{query}'")
    print(f"Filter: {{'is_footnote': False, 'paragraph_id': {{'$gt': 5}}}}")
    ...
    for i, doc in enumerate(results, 1):
        ...
        print(f"  Metadata: {doc.metadata}")
        # Verify filters
        assert doc.metadata.get("is_footnote") == False, "Filter failed!"
...
def main() -> None:
    ...
    try:
        ...
        test_metadata_filter_operators(
            vector_store,
            "programming and computers"
        )
        ...
        try:
            ...
            def retrieve_with_filter(
                ...
                Args:
                    query: The search query.
                    is_footnote: Filter by footnote status (True for footnotes, False for regular paragraphs, None for all).
            ...
            prompt = (
                "You have access to a tool that retrieves context from a document with metadata filtering. "
                "The document contains paragraphs and footnotes. "
                "You can filter by footnote status (is_footnote) and paragraph ID range. "
                "Use the tool to help answer user queries, and use filters when the user asks for specific types of content."
            )
        ...
        query4 = "Find information about Lisp, but exclude footnotes and only search in paragraphs 5-20."
